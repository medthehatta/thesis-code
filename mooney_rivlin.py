...
def iso_material_elasticity_2(F,H,*params): 
    ...
    Ci = np.linalg.inv(C)
    I = np.eye(3)
    So = iso_material_model(F,H,*params)
    ...
    # Compute PPSo
    PPSo = np.einsum('abcd,cd',PP,So)

    # Compute CivPPSo
    CixPPSo = np.einsum(tensor,Ci,PPSo)
    PPSoxCi = np.einsum(tensor,PPSo,Ci)
    CivPPSo = CixPPSo + PPSoxCi

    part1 = 4*c01*np.einsum('abcd,cdef,efgh',PP,(IxI - IsI),PPt)
    part2 = (2/3.)*np.tensordot(So,C)*PPx
    part3 = -(2/3.)*CivPPSo
